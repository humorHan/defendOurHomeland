 ...
 */
var GameOverLayerFail = cc.Layer.extend({
    ctor: function(){
        this._super();
        ...
        this.runAction(seq);
        //添加胜利消息
        var message = new cc.LabelTTF("完败","American Typewriter",50,"yellow",cc.TEXT_ALIGNMENT_LEFT);
        message.attr({
            x: 400,
            y: 300
        });
...
var GameOverSceneFail = cc.Scene.extend({
    onEnter: function(){
        this._super();
        var gameOverLayerFail = new GameOverLayerFail();
        this.addChild(gameOverLayerFail);
    }
});
